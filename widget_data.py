...
def quota_windows(data):
    ...
    for row in limits:
        ...
        if minutes in (300,10080):
            found[minutes] = row
    return [('周额度' if minutes == 10080 else '5 小时额度', found[minutes])
            for minutes in (10080,300) if minutes in found]


def desktop_snapshot(coordinator):
    data = coordinator.snapshot()
    result = {key:data[key] for key in ('total','by_model','windows','charts','pricing','source','generated_at','index') if key in data}
    result['rate_limits'] = {'limits':[]}
    index = getattr(coordinator,'index',None)
    if index is not None:
        try:
            with index.read_transaction() as reader:
                row=reader.execute('''SELECT q.payload_json FROM quota_cache q
                    JOIN session_files f ON q.file_id=f.file_id WHERE f.missing=0
                    ORDER BY q.observed_at DESC,q.source_offset DESC LIMIT 1''').fetchone()
                if row:
                    result['rate_limits']=json.loads(row['payload_json'])
                else:
                    row=reader.execute('''SELECT r.observed_at,r.used_percent,r.window_minutes,r.resets_at
                        ...
                        ORDER BY r.observed_at DESC,r.source_offset DESC LIMIT 1''').fetchone()
                    if row:
                        primary={key:row[key] for key in ('observed_at','used_percent','window_minutes','resets_at')}
