...
from glass_popups import GlassInfoPopup
...
class GlassChart(QWidget):
    ...
    def __init__(self,parent=None,color='#149cff',mini=False):
        ...
        self.color=QColor(color); self.mini=mini; self.labels=[];self.show_endpoint=False
        self.target_values=[]; self.display_values=[]; self.start_values=[]
        self.unit='Token / min'; self.motion=True; self.live=False; self.phase=0; self.hover_index=None
        self.axis_font_size=13; self.y_axis=False
        self._view_start=0.0; self._view_end=0.0
        self.info_popup=GlassInfoPopup(self)
        self.animation=QVariantAnimation(self); self.animation.setDuration(620)
        ...
        self.pulse=QTimer(self); self.pulse.setInterval(40); self.pulse.timeout.connect(self.tick)
    ...
    def set_series(self,values,labels=None,animate=True,unit='Token / min'):
        ...
        old_labels=self.labels[:]; old_unit=self.unit; old_count=len(self.target_values)
        ...
        self.labels=new_labels; self.unit=unit
        self._preserve_or_reset_view(old_labels,new_labels,old_unit,unit,old_count,len(clean),old_view,was_zoomed)
        if not values_changed:
            self.update(); return
        self.animation.stop()
        old=self.display_values[:]
        if not old or not animate or not self.motion or not clean:
            self.display_values=clean[:]; self.update(); return
    ...
    def _preserve_or_reset_view(self,old_labels,new_labels,old_unit,new_unit,old_count,new_count,old_view,was_zoomed):
        if new_count<=1:
    ...
    def _set_view(self,start,end,emit=True):
        ...
        if full<=0:
            ...
            start=float(start); end=start+span
            if start<0:
                end-=start;start=0.0
            if end>full:
                start-=end-full;end=full
            self._view_start=max(0.0,start);self._view_end=min(full,end)
    ...
    def set_view_range(self,start,end):
        ...
        count=len(self.target_values)
        if count<2:
            self.reset_zoom();return self.view_range
        ...
        if end-start<minimum:
            center=(start+end)/2
            start=center-minimum/2;end=center+minimum/2
    ...
    def interpolate(self,value):
        ...
        if t>=1:self.display_values=self.target_values[:]
        self.update()

    def set_live(self,live):
        self.live=bool(live)
        if self.live and self.motion and self.isVisible() and not self.mini:self.pulse.start()
        else:self.pulse.stop()
        self.update()

    def tick(self):
        self.phase=(self.phase+.08)%(2*math.pi); self.update()

    def hideEvent(self,event):
        self.pulse.stop(); self.info_popup.hide(); super().hideEvent(event)

    def showEvent(self,event):
        super().showEvent(event); self.set_live(self.live)

    def plot_rect(self):
        left=3
        if self.y_axis and not self.mini:
            from PyQt5.QtGui import QFontMetrics
            f=QFont('Arial');f.setPixelSize(self.axis_font_size);metrics=QFontMetrics(f)
            left=max(52,max(metrics.horizontalAdvance(self._compact_number(self._visible_peak()*i/4)) for i in range(5))+8)
    ...
    def path_for(self,values,rect,indices=None,peak=None):
        path=QPainterPath()
        if not values:return path,[]
        if indices is None:indices=list(range(len(values)))
        ...
        points=[QPointF(rect.left()+(i-self._view_start)*rect.width()/span,rect.bottom()-v/peak*rect.height()) for i,v in zip(indices,values)]
    ...
    def _visible_peak(self):
        if not self.target_values:return 1.0
    ...
    def _format_axis_label(self,index):
        if index>=len(self.labels):return str(index)
    ...
    def paintEvent(self,event):
        ...
        rect=self.plot_rect(); peak=self._visible_peak()
        first=max(0,int(math.floor(self._view_start))-1);last=min(len(self.display_values)-1,int(math.ceil(self._view_end))+1)
        ...
        if not self.mini:
            font=QFont('Arial');font.setPixelSize(self.axis_font_size);p.setFont(font)
            p.setPen(QPen(QColor(255,255,255,80),.6))
            for i in range(7):
                x=rect.left()+rect.width()*i/6;p.drawLine(QPointF(x,rect.top()),QPointF(x,rect.bottom()))
            if self.y_axis:
                for i in range(5):
                    y=rect.bottom()-rect.height()*i/4
                    p.drawLine(QPointF(rect.left(),y),QPointF(rect.right(),y))
                    p.setPen(QColor('#46638d'))
                    p.drawText(QRectF(0,y-10,rect.left()-7,20),Qt.AlignRight|Qt.AlignVCenter,self._compact_number(peak*i/4))
                    p.setPen(QPen(QColor(255,255,255,80),.6))
            p.setPen(QPen(QColor(255,255,255,170),1));p.drawLine(rect.bottomLeft(),rect.bottomRight())
        if points:
            p.save();p.setClipRect(rect.adjusted(-1,-1,1,1))
            fill=QPainterPath(path);fill.lineTo(points[-1].x(),rect.bottom());fill.lineTo(points[0].x(),rect.bottom());fill.closeSubpath()
            gradient=QLinearGradient(0,rect.top(),0,rect.bottom())
            top=QColor(self.color);top.setAlpha(190 if not self.mini else 160)
            middle=QColor(self.color);middle.setAlpha(100 if not self.mini else 76)
            bottom=QColor(self.color);bottom.setAlpha(12 if not self.mini else 6)
            gradient.setColorAt(0,top);gradient.setColorAt(.55,middle);gradient.setColorAt(1,bottom)
            p.fillPath(fill,gradient)
            glow=QColor(self.color);glow.setAlpha(24)
            p.setPen(QPen(glow,7 if not self.mini else 5,Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin));p.drawPath(path)
            p.setPen(QPen(QColor(255,255,255,180),3.8 if not self.mini else 2.7,Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin));p.drawPath(path)
            p.setPen(QPen(self.color,2.4 if not self.mini else 2.1,Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin));p.drawPath(path)
            ...
            if not self.mini or self.show_endpoint:
                end=points[endpoint_pos];p.setPen(QPen(QColor(255,255,255,200),1,Qt.DashLine));p.drawLine(QPointF(end.x(),rect.top()),QPointF(end.x(),rect.bottom()))
                radius=7+2*math.sin(self.phase) if self.live and self.motion else 5
                halo=QColor(self.color);halo.setAlpha(35);p.setPen(Qt.NoPen);p.setBrush(halo);p.drawEllipse(end,radius,radius)
                p.setPen(QPen(Qt.white,1.5));p.setBrush(self.color);p.drawEllipse(end,3.2,3.2)
            if self.hover_index is not None and self.hover_index in indices:
                point=points[indices.index(self.hover_index)];p.setPen(QPen(QColor('#7095bd'),.8,Qt.DashLine));p.drawLine(QPointF(point.x(),rect.top()),QPointF(point.x(),rect.bottom()))
                p.setPen(QPen(Qt.white,1.5));p.setBrush(self.color);p.drawEllipse(point,3,3)
            p.restore()
        if not self.mini:
            font=QFont('Arial');font.setPixelSize(self.axis_font_size);p.setFont(font);p.setPen(QColor('#46638d'))
            if self.labels:
                indexes=self._axis_indexes()
                for j,i in enumerate(indexes):
                    text=self._format_axis_label(i)
                    width=80;x=rect.left() if j==0 else (rect.right()-width if j==2 else rect.center().x()-width/2)
    ...
    def mouseMoveEvent(self,event):
        if not self.target_values or self.mini:return
        rect=self.plot_rect()
        if not rect.contains(event.pos()):
            self.hover_index=None;self.info_popup.hide();self.update();return
        anchor=max(0.0,min(1.0,(event.x()-rect.left())/max(1,rect.width())))
        sample=self._view_start+anchor*(self._view_end-self._view_start)
        first,last=self._viewport_indices();i=max(first,min(last,round(sample)))
        self.hover_index=i; self.update()
        i=self.hover_index;stamp=self.labels[i] if i<len(self.labels) else ''
        self.info_popup.show_info(stamp,self.target_values[i],self.unit,event.globalPos()+QPoint(12,12))

    def leaveEvent(self,event):
        self.hover_index=None;self.info_popup.hide();self.update()
    ...
    def wheelEvent(self,event):
        ...
        self.info_popup.hide();self.zoom_at(anchor,delta/120.0);event.accept()

    def mouseDoubleClickEvent(self,event):
        if not self.mini and event.button()==Qt.LeftButton and self.plot_rect().contains(event.pos()):
            self.info_popup.hide();self.reset_zoom();event.accept();return
        event.ignore()
