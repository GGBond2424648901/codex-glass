...
from codex_monitor_index import IndexCoordinator, IndexStatus, SessionIndex, default_index_path, history_page
from monitor import configure_console_output


_DASHBOARD_BUILD = datetime.fromtimestamp(
    (Path(sys.executable) if getattr(sys, 'frozen', False) else Path(__file__)).stat().st_mtime
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .index-copy{flex:1 1 240px;min-width:0}
    .index-details{font-size:12px;line-height:1.8;color:var(--muted)}
    .index-banner progress{width:140px;max-width:100%;accent-color:var(--warning)}
    .index-banner.is-error{border-color:var(--danger);color:var(--danger)}
...
<body>
  <div class="shell">
      ...
      <div class="hero-card">
        <div class="hero-top">
          ...
          <div class="hero-slab">
            ...
            <div class="signal-pod">
              <span class="key">已知价格估算</span>
              <span class="val mono" id="heroCost">-</span>
              <span class="sub" id="heroCostDetail">仅包含有已知价格的用量</span>
            </div>
            <div class="signal-pod">
    ...
    <div class="index-banner" id="indexBanner" role="status" aria-live="polite">
      <div class="index-copy">
        <div id="indexMessage">数据索引中，当前总计尚不完整</div>
        <div class="index-details" id="indexDetails">正在准备索引...</div>
      </div>
      <progress id="indexProgress" max="100" value="0" aria-label="索引进度"></progress>
      <button class="secondary" id="btnIndexRetry" type="button" hidden>重试索引</button>
    </div>
    ...
    <section class="board">
      <section class="tactical-band">
        <article class="cluster-card cost">
          ...
          <div class="cluster-grid">
            ...
            <div class="cluster-stat">
              <span class="k">价格覆盖</span>
              <span class="v mono" id="pricingCoverage">-</span>
            </div>
            <div class="cluster-stat">
              <span class="k">调用价格覆盖</span>
              <span class="v mono" id="pricingCallCoverage">-</span>
            </div>
            <div class="cluster-stat">
              <span class="k">未定价用量</span>
              <span class="v mono bounded-detail" id="unpricedUsage">-</span>
            </div>
          </div>
        </article>
        ...
        <div class="panel-header">
          ...
          <div class="control-row">
            <select id="chartMetric" aria-label="指标">
              ...
              <option value="cost">已知价格估算（美元）</option>
      ...
      <div class="side-column">
        <aside class="drilldown-panel" id="drilldownPanel">
          ...
          <div class="drilldown-note" id="drilldownNote">悬停可查看模型、槽位、工作区与已知价格估算；按 F 可切换大屏，Esc 退出。</div>
        </aside>
  ...
  <script>
    ...
    function renderDrilldown(spec){
      ...
      setText('drilldownNote', payload.note || '悬停可查看模型、槽位、工作区与已知价格估算。');
    ...
    function renderDefaultDrilldown(data, metrics){
      ...
      renderDrilldown({
        ...
        stats: [
          {label: '已知价格估算', value: fmtUsdCompact(((data.total || {}).estimated_cost_usd) || 0)},
          ...
          return {
            ...
            drilldown: {
              ...
              stats: [
                ...
                {label: '已知价格估算', value: fmtUsd(row.estimated_cost_usd || 0)},
                {label: '输入 / 输出', value: `${fmtInt(row.input_tokens || 0)} / ${fmtInt(row.output_tokens || 0)}`},
              ],
              note: '来自当前主图时间桶，可用来对比脉冲、已知价格估算和事件密度。',
            },
          };
        },
      });
        ...
        hitTest: ({y, height}) => {
          ...
          return {
            ...
            drilldown: {
              ...
              note: '模型负载排行 Drill-down，可快速识别已知价格估算较高或高吞吐模型。',
            },
          };
        },
      });
    ...
    function renderOverview(data, metrics){
      ...
      const importedSources = Number(source.imported_sources || 0) || 0;
      const importedFiles = Number(source.imported_files || 0) || 0;
      const importedEvents = Number(source.imported_usage_events || 0) || 0;
      const domainLabel = importedSources
        ? `${source.label || '本机会话遥测'} + ${fmtInt(importedSources)} 个导入来源`
        : (source.label || '本机会话遥测');
      ...
      setText('meta', `更新时间：${data.generated_at || '-'} · 本机文件 ${fmtInt(source.files || 0)} · 导入文件 ${fmtInt(importedFiles)} · 活跃模型 ${fmtInt(metrics.activeModels)} · 构建 ${server.build || '-'}`);
      ...
      setText('heroCostDetail', `仅含已知价格用量 · 令牌覆盖 ${fmtPercent(pricing.token_coverage_percent)}`);
      ...
      setText('sampleFiles', importedSources
        ? `本机 ${fmtInt(source.files || 0)} · 导入 ${fmtInt(importedFiles)} 文件 / ${fmtInt(importedEvents)} 事件`
        : `${fmtInt(source.files || 0)} 个日志文件`);
      ...
      setText('pricingCoverage', `${fmtPercent(pricing.token_coverage_percent)} · ${fmtInt(pricing.priced_tokens || 0)} / ${fmtInt(pricing.total_tokens || 0)} tok`);
      setText('pricingCallCoverage', `已知价格估算 ${fmtInt(pricing.priced_calls || 0)} / ${fmtInt(pricing.total_calls || 0)} 次 · 未定价 ${fmtInt(pricing.unpriced_calls || 0)} 次`);
      const unpricedModels = asArray(pricing.unpriced_models);
      const unpricedDetails = unpricedModels.length
        ? unpricedModels.map((row) => `${row.model || 'unknown'} ${fmtInt(row.total_tokens || 0)} tok / ${fmtInt(row.calls || 0)} 次`).join(' · ')
        : '无';
      setText('unpricedUsage', `${fmtInt(pricing.unpriced_tokens || 0)} tok · ${unpricedDetails}`);
      setText('note', `美元总额仅包含有已知价格的用量；按当前价格重估，不是历史账单。定价核对 ${pricing.policy_checked_on || '-'}。`);
    ...
    function renderWindows(data, metrics){
      ...
      const h5HourRows = h5Rows.map((row) => {
        ...
        bindDrilldown(tr, () => ({
          ...
          note: '严格 rolling 5h 明细，适合查看每个桶的已知价格估算与负载结构。',
        }));
        return tr;
      });
      ...
      const rows = events.map((event) => {
        ...
        bindDrilldown(tr, () => ({
          ...
          stats: [
            ...
            {label: '已知价格估算', value: fmtUsd((event.cost_usd || {}).total || 0)},
    ...
    function formatIndexBytes(value){
    ...
    function renderIndex(index){
      ...
      banner.hidden = state.complete === true && state.status === 'ready';
      banner.classList.toggle('is-error', failed);
      document.getElementById('btnIndexRetry').hidden = !failed;
      const message = failed
        ? `索引暂时失败，当前总计尚不完整：${state.last_error || '请重试'}`
        : '数据索引中，当前总计尚不完整';
      setText('indexMessage', message);
      const percent = Math.max(0, Math.min(100, Number(state.progress_percent) || 0));
      const current = state.current_file ? ` · ${state.current_file}` : '';
      const waiting = state.status === 'waiting' ? ' · 等待索引进程' : '';
      setText('indexDetails', `已索引文件 ${fmtInt(state.processed_files || 0)} / ${fmtInt(state.total_files || 0)} · ${formatIndexBytes(state.processed_bytes)} / ${formatIndexBytes(state.total_bytes)} · ${percent.toFixed(1)}%${current}${waiting}`);
      document.getElementById('indexProgress').value = percent;
    }
    ...
    document.getElementById('btnIndexRetry').addEventListener('click', refreshNow);
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/widget":
            from widget_data import desktop_snapshot
        ...
        if parsed.path == '/api/native-context':
            ...
            if not ipaddress.ip_address(self.client_address[0]).is_loopback or host not in ('127.0.0.1','localhost','::1') or self.headers.get('Origin'):
                self._send(403,b'local native client only');return
            ...
            if index is not None:
                ...
                try:
                    with index.read_transaction() as reader:
                        data['sources']=[dict(row) for row in reader.execute('SELECT source_id,first_imported_at,source_files,source_usage_events,source_rate_limit_snapshots FROM history_imports ORDER BY first_imported_at')]
                except (RuntimeError,sqlite3.Error):pass
            self._send(200,json.dumps(data).encode(),'application/json; charset=utf-8');return
        ...
        if parsed.path == "/api/events":
            ...
            for name in ('since','until','sort'):
                if name in qs:filters[name]=qs[name][0]
            if 'models' in qs:filters['models']=[m for item in qs['models'] for m in item.split(',') if m]
            if qs.get('empty_models')==['1']:filters['models']=[]
            if 'descending' in qs:filters['descending']=qs['descending'][0]!='0'
            try:
                payload = self.server.coordinator.events_page(offset=offset, limit=limit, q=q, model=model, cwd=cwd,**filters)
            except ValueError:
                self._send(400,b'invalid history date');return
    ...
    def snapshot(self):
        with self._lock:
            if self._summary is None or time.monotonic() - self._updated_at >= self.interval:
                self._summary = build_usage_summary(self.sessions_dir, self.config_loader(),
                                                    self.cwd_filter, include_events=True)
                self._summary["index"] = IndexStatus(
                    "ready", True, self._summary["source"]["files"], self._summary["source"]["files"],
                    0, 0, None, None, None, None, time.time()).to_payload()
...
def main():
    ...
    try:
        if args.no_index:
            coordinator = LegacyCoordinator(sessions_dir, lambda: MonitorConfig.load(config_path),
                                            args.cwd, interval=args.update_interval)
        else:
            index_path = (Path(args.index_db).expanduser() if args.index_db else default_index_path()).resolve()
            index = SessionIndex(index_path)
            coordinator = IndexCoordinator(index, sessions_dir, lambda: MonitorConfig.load(config_path),
                                           args.cwd, interval=args.update_interval, rebuild_index=args.rebuild_index)
