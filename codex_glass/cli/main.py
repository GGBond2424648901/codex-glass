...
from codex_glass.core.usage import DEFAULT_HOST, DEFAULT_PORT, MonitorConfig
from codex_glass.storage.history_import import HistoryImportError, HistoryImporter
from codex_glass.storage.index import IndexStatus, SCHEMA_VERSION, SessionIndex, default_index_path
from codex_glass.resources import dashboard_build_identity, resource_path, resource_root


LAUNCHER_NAME = "monitor.py"
...
def _package_environment() -> dict[str, str]:
    ...
    project_root = str(resource_root())
    existing = environment.get("PYTHONPATH")
    environment["PYTHONPATH"] = project_root if not existing else project_root + os.pathsep + existing
...
def _local_dashboard_build() -> Optional[str]:
    try:
        return dashboard_build_identity()
    except Exception:
        return None
...
def open_dashboard(args) -> int:
    ...
    print(f"🌐 仪表板地址: {url}")
    print(f"关闭命令: python3 {LAUNCHER_NAME} stop")
    if args.no_browser:
        return 0
...
def run_background(args) -> int:
        ...
        try:
            ...
            if not healthy or not is_monitor:
                ...
                if stop_rc != 0:
                    ...
                    print(f"停止命令: python3 {LAUNCHER_NAME} stop")
                    print(f"地址: {url}")
                    return stop_rc
                ...
                print(f"地址: {url}")
                print(f"关闭命令: python3 {LAUNCHER_NAME} stop")
                print(f"重新打开: python3 {LAUNCHER_NAME} open")
    ...
    if _wait_for_server(url, timeout_seconds=10):
        ...
        print(f"日志: {log_file}")
        print(f"前台排查: python3 {LAUNCHER_NAME} web")
        return 1
...
def run_enhanced_terminal(args) -> int:
    """启动增强版终端监控"""
    enhanced_script = Path(__file__).with_name("enhanced.py")
    ...
    cmd = [sys.executable, "-m", "codex_glass.cli.enhanced"]
    if args.sessions_dir:
        cmd += ["--sessions-dir", args.sessions_dir]
    if args.config:
        cmd += ["--config", args.config]
    ...
    print("🚀 启动增强版终端监控...")
    try:
        return subprocess.run(cmd, env=_package_environment()).returncode
...
def run_simple_terminal(args) -> int:
    """启动标准版终端监控"""
    simple_script = Path(__file__).with_name("simple.py")
    if not simple_script.exists():
        print(f"❌ 找不到文件: {simple_script}")
        return 1

    cmd = [sys.executable, "-m", "codex_glass.cli.simple"]
    if args.sessions_dir:
        cmd += ["--sessions-dir", args.sessions_dir]
    if args.config:
        cmd += ["--config", args.config]
