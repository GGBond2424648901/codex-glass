...
def font(size, bold=False):
    ...
    result.setWeight(QFont.Bold if bold else QFont.Normal)
...
def label(parent, text, size=12, color=INK, bold=False):
    ...
    item.setStyleSheet("color:" + color + ";background:transparent;")
    return item
...
def pulse_path(rect):
    p = QPainterPath()
    points = [(0, 0.53), (0.13, 0.53), (0.25, 0.2), (0.36, 0.68), (0.49, 0.04), (0.67, 0.92), (0.81, 0.48), (1, 0.48)]
...
class IconButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        if self.kind == "more":
            ...
            path.closeSubpath()
            p.drawPath(path)
            p.drawLine(14, 16, 14, 23)
        elif self.kind == "down":
            p.drawLine(8, 11, 14, 17)
            p.drawLine(14, 17, 20, 11)
        elif self.kind == "up":
            p.drawLine(8, 17, 14, 11)
            p.drawLine(14, 11, 20, 17)
...
class Appearance(QWidget):
    def __init__(self, owner):
        ...
        self.setFixedSize(330, 323)
        self.setStyleSheet(STYLE)
        retained = getattr(owner, "_glass_popup_children", None)
        if retained is None:
            retained = []
            owner._glass_popup_children = retained
        ...
        self.value = label(self, "65%", 14)
        self.value.setAlignment(Qt.AlignRight)
        self.value.setGeometry(242, 65, 60, 22)
        self.opacity_slider = QSlider(Qt.Horizontal, self)
        self.opacity_slider.setRange(15, 90)
        self.opacity_slider.setValue(owner.transparency)
        self.opacity_slider.setGeometry(28, 88, 274, 28)
        self.opacity_slider.setAccessibleName("玻璃透明度")
        self.opacity_slider.valueChanged.connect(self.change_opacity)
        self.presets = {}
        for i, (text, key) in enumerate((("通透", "clear"), ("平衡", "balanced"), ("磨砂", "frosted"))):
            button = GlassPreset(text, key, self)
            button.setCheckable(True)
            button.setChecked(key == owner.glass_preset)
            button.setGeometry(28 + i * 94, 143, 86, 99)
            button.setFont(font(14))
            button.clicked.connect(lambda checked=False, k=key: self.select_preset(k))
            self.presets[key] = button
        label(self, "始终置顶", 14).setGeometry(28, 278, 190, 25)
        self.top = GlassSwitch(self)
        self.top.setGeometry(252, 277, 50, 27)
        self.top.setChecked(True)
        self.top.toggled.connect(owner.top_action.setChecked)

    def change_opacity(self, value):
        self.value.setText(f"{value}%")
        self.owner.set_transparency(value)
        self.update()
    ...
    def showEvent(self, event):
        ...
        native_blur(self)
        self.change_opacity(self.owner.transparency)
    ...
    def paintEvent(self, event):
        ...
        material(
            ...
            self.owner.transparency,
            self.owner.glass_preset,
            self.owner.flow_phase,
        )
        p.setPen(QColor(255, 255, 255, 150))
        p.drawLine(28, 125, 302, 125)
        p.drawLine(28, 258, 302, 258)
...
class GlassSwitch(QCheckBox):
    ...
    def paintEvent(self, event):
        ...
        p.setRenderHint(QPainter.Antialiasing)
        p.setPen(Qt.NoPen)
        p.setBrush(QColor("#159cff" if self.isChecked() else "#a6b8d4"))
        p.drawRoundedRect(QRectF(1, 1, 48, 25), 12.5, 12.5)
        p.setBrush(QColor(255, 255, 255, 245))
        p.drawEllipse(QRectF(25 if self.isChecked() else 3, 3, 21, 21))
...
class GlassRange(QComboBox):
    ...
    def showPopup(self):
        ...
        if self.popup_surface is None or self.popup_surface.options != options:
            if self.popup_surface is not None:
                release_owned_popup(self, self.popup_surface)
            self.popup_surface = GlassChoicePopup(options, self.currentText(), self._choose, self)
        else:
            self.popup_surface.selection = self.currentText()
        surface = self.window()
        point = QPoint(0, self.height() + 6)
        global_point = (
            surface.global_point(self, point) if hasattr(surface, "global_point") else self.mapToGlobal(point)
        )
    ...
    def paintEvent(self, event):
        ...
        p.drawRoundedRect(QRectF(0.5, 0.5, self.width() - 1, self.height() - 1), 17, 17)
        p.setFont(font(14))
        p.setPen(QColor("#274f82"))
...
class GlassPreset(QPushButton):
    def __init__(self, text, key, parent=None):
        ...
        self.key = key
        self.setAccessibleName(text + "玻璃")
        self.setCursor(Qt.PointingHandCursor)

    def paintEvent(self, event):
        p = QPainter(self)
        p.setRenderHint(QPainter.Antialiasing)
        rect = QRectF(3, 2, 80, 71)
        gradient = QLinearGradient(rect.topLeft(), rect.bottomRight())
        colors = ["#f1cfd2", "#bfd7f7", "#a3dce9"] if self.key != "frosted" else ["#dddce9", "#d5e1ef", "#cde8ec"]
        for t, c in zip((0, 0.5, 1), colors):
            gradient.setColorAt(t, QColor(c))
        p.setBrush(gradient)
        p.setPen(QPen(QColor(BLUE if self.isChecked() else "#f3f5ff"), 1.5))
        p.drawRoundedRect(rect, 12, 12)
        p.setPen(QColor(INK))
        p.setFont(font(13))
        p.drawText(QRectF(0, 79, 86, 20), Qt.AlignCenter, self.text())
