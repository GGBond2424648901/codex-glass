...
def app_icon():
    path = resource_path("assets", "codex-glass.png")
    if path.exists():
    ...
    p.drawRoundedRect(3, 3, 58, 58, 16, 16)
    p.setPen(QPen(Qt.white, 4, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
    p.drawPath(pulse_path(QRectF(12, 17, 40, 30)))
    p.end()
    return QIcon(pix)
...
class ModelRow(QWidget):
    def __init__(self, name, color, callback):
        ...
        self.unpriced = False
        self.separator = True
        self.chart = GlassChart(self, color, mini=True)
        self.chart.setAttribute(Qt.WA_TransparentForMouseEvents)
        self.setFixedHeight(65)
        self.setCursor(Qt.PointingHandCursor)
        self.setFocusPolicy(Qt.StrongFocus)
        self.setAccessibleName(name + " 模型详情")

    def refresh(self, stats, values, unpriced, animate=True):
        self.stats = stats
        self.unpriced = unpriced
        self.chart.set_series(values, animate=animate)
        self.setToolTip(
            self.name
            + f'\n{stats.get("total_tokens",0):,} Token\n'
            + ("未定价，不计入费用" if unpriced else f'预估 ${stats.get("estimated_cost_usd",0):,.2f}')
        )
        self.update()
    ...
    def paintEvent(self, event):
        ...
        if self.underMouse() or self.hasFocus():
            ...
            p.setPen(Qt.NoPen)
            p.drawRoundedRect(QRectF(0, 1, w, h - 2), 10, 10)
        p.setPen(Qt.NoPen)
        p.setBrush(QColor(self.color))
        p.drawEllipse(QRectF(0, mid - 6.5, 13, 13))
        p.setFont(font(17, True))
        p.setPen(QColor(INK))
        p.drawText(
            QRectF(25, mid - 14, 108, 28),
            Qt.AlignVCenter,
            p.fontMetrics().elidedText("未定价模型" if self.name == "unknown-model" else self.name, Qt.ElideRight, 107),
        )
        p.setFont(font(16, True))
        p.drawText(
            QRectF(w - 93, mid - 21, 90, 23),
            Qt.AlignRight | Qt.AlignVCenter,
            display_number(self.stats.get("total_tokens")),
        )
        p.setFont(font(15))
        p.setPen(QColor(MUTED))
        cost = "未计价" if self.unpriced else f'${self.stats.get("estimated_cost_usd",0):,.2f}'
...
class GlassWidget(QWidget):
    ...
    def __init__(self, url="http://127.0.0.1:8081", start_backend=True, persist=True):
        ...
        self.settings = QSettings("CodexMonitor", "FrostedGlass")
        ...
        self.setWindowTitle("Codex Glass")
        ...
        if persist:
            ...
            if self.glass_preset not in ("clear", "balanced", "frosted"):
                self.glass_preset = "balanced"
        self.body = QWidget(self)
        self.body.setGeometry(0, 54, 440, 632)
        self.brand = label(self, "Codex", 22, bold=True)
        self.brand.setGeometry(82, 18, 78, 32)
        self.dot = label(self, "●", 18, MINT)
        self.dot.setGeometry(155, 21, 18, 23)
        self.pin = IconButton("pin", "始终置顶", self)
        ...
        self.segments = QWidget(self.body)
        self.segments.setGeometry(28, 4, 384, 42)
        self.segments.setStyleSheet(
            "background:rgba(91,104,159,22);border:1px solid rgba(255,255,255,75);border-radius:20px;"
        )
        self.group = QButtonGroup(self)
        self.group.setExclusive(True)
        for i, (title, key) in enumerate((("今日", "today"), ("5小时", "last_5_hours"), ("累计", "all"))):
            b = QPushButton(title, self.body)
            b.setGeometry(31 + 127 * i, 7, 124, 36)
            b.setCheckable(True)
            b.setChecked(i == 0)
            b.setFont(font(16, i == 0))
            b.setStyleSheet(
                "QPushButton{border:0;border-radius:18px;background:transparent;} QPushButton:checked{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,stop:0 rgba(255,255,255,235),stop:1 rgba(240,249,255,215));border:1px solid white;} QPushButton:hover{background:rgba(255,255,255,125);}"
            )
        ...
        self.tokens_title = label(self, "Token", 16, MUTED)
        self.tokens_title.setGeometry(284, 119, 121, 24)
        self.tokens = label(self, "—", 32, bold=True)
        self.tokens.setGeometry(283, 145, 124, 43)
        self.cost.setToolTip("按已知模型价格估算，非实际账单；未定价用量不计入费用。")
        self.chart_label = label(self.body, "Token / min", 16, MUTED)
        self.chart_label.setGeometry(32, 154, 205, 25)
        self.rate = label(self.body, "—", 25, BLUE, True)
        self.rate.setAlignment(Qt.AlignRight)
        self.rate.setGeometry(284, 152, 123, 29)
        self.range_box = GlassRange(self.body)
        self.range_box.addItems(["近7天", "近30天", "近90天"])
        self.range_box.setCurrentIndex(1)
        self.range_box.setGeometry(315, 150, 90, 33)
        self.range_box.setFont(font(14))
        self.range_box.currentIndexChanged.connect(lambda: self.render_chart(False))
        self.range_box.hide()
        self.chart = GlassChart(self.body)
        ...
        self.collapse.move(385, 572)
        self.collapse.clicked.connect(self.toggle_compact)
        self.expand = IconButton("up", "展开完整组件", self)
        self.expand.move(300, 17)
        self.expand.clicked.connect(self.toggle_compact)
        self.expand.hide()
        self.mini_chart = GlassChart(self, mini=True)
        ...
        self.menu.addAction("显示 / 恢复", self.restore)
        self.menu.addAction("收起到托盘", self.hide_to_tray)
        self.top_action = self.menu.addAction("始终置顶")
        self.top_action.setCheckable(True)
        self.top_action.setChecked(True)
        self.top_action.toggled.connect(self.toggle_top)
        self.pin.toggled.connect(self.top_action.setChecked)
        self.menu.addAction("迷你 / 展开", self.toggle_compact)
        self.menu.addAction("外观…", self.show_appearance)
        self.motion_action = self.menu.addAction("动态效果")
        ...
        self.tray.setToolTip("Codex Glass")
        self.tray.setContextMenu(self.menu)
        self.tray.activated.connect(self.tray_activate)
        self.tray.show()
        self.manager = QNetworkAccessManager(self)
        self.manager.finished.connect(self.received)
        ...
        if persist:
            ...
            if pos and any(s.availableGeometry().contains(pos) for s in QApplication.screens()):
                self.move(pos)
        self.flash = QVariantAnimation(self)
        self.flash.setDuration(750)
        self.flash.setStartValue(0.0)
        self.flash.setEndValue(1.0)
        self.flash.valueChanged.connect(self.flash_metric)
        ...
        if sys.platform == "win32":
            ...
            try:
                if (
                    ...
                    and not enabled.value
                ):
                    self.motion_action.setChecked(False)
    ...
    def showEvent(self, event):
        ...
        self.blur_active = native_blur(self)
        if hasattr(self, "timer"):
            self.timer.start()
        if hasattr(self, "flow_timer") and self.motion:
    ...
    def paintEvent(self, event):
        ...
        material(
            p,
            QRectF(5, 5, self.width() - 10, self.height() - 10),
            self.transparency,
            self.glass_preset,
            self.flow_phase,
        )
        p.setPen(QPen(QColor(BLUE), 2.5, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
        p.drawPath(pulse_path(QRectF(36, 22, 31, 27)))
        ...
        p.drawLine(32, 600, 408, 600)
        track = QRectF(32, 643, 250, 13)
        p.setPen(Qt.NoPen)
        p.setBrush(QColor(93, 132, 175, 58))
        p.drawRoundedRect(track, 6.5, 6.5)
        if self.remaining is not None:
            p.setBrush(QColor(MINT if self.remaining > 20 else "#e9aa65"))
            p.drawRoundedRect(
                QRectF(track.x(), track.y(), track.width() * self.remaining / 100, track.height()), 6.5, 6.5
            )
    ...
    def render(self):
        if not self.data:
            return
        total, models = window_data(self.data, self.scope)
        animate = self.chart_scope == self.scope
        ...
        self.cost.setText("—" if incomplete else f'${total.get("estimated_cost_usd",0):,.2f}')
        self.tokens.setText("—" if incomplete else display_number(total.get("total_tokens")))
        self.fit_metrics()
        ...
        ordering = sorted(
            ...
            key=lambda n: (
                n in unpriced,
                0 if n == "gpt-6-astra" else 1 if n == "gpt-5.6-sol" else 2,
                -models[n].get("total_tokens", 0),
                n,
            ),
        )
        ...
        for i, name in enumerate(ordering):
            if name not in self.rows:
                self.rows[name] = ModelRow(
                    name, colors.get(name, "#9798b5" if name in unpriced else "#739de2"), self.select_model
                )
            row = self.rows[name]
            row.separator = i < len(ordering) - 1
            self.model_layout.removeWidget(row)
            self.model_layout.insertWidget(i, row)
            row.setFixedHeight(49 if self.scope == "all" else 63)
            row.chart.motion = self.motion
            row.refresh(models[name], series.get("by_model", {}).get(name, []), name in unpriced, animate)
        self.chart_scope = self.scope
        self.render_quota()
        self.update_status()
        self.update()
    ...
    def fit_metrics(self):
        ...
        while (
            chosen > 19 and QFontMetrics(font(chosen, True)).horizontalAdvance(self.cost.text()) > self.cost.width() - 4
        ):
            chosen -= 1
        self.cost.setFont(font(chosen, True))
        size = 23 if self.compact_mode else 32
        while size > 14 and QFontMetrics(font(size, True)).horizontalAdvance(self.tokens.text()) > self.tokens.width():
            size -= 1
    ...
    def current_series(self):
        ...
        return {
            **chart,
            "labels": chart.get("labels", [])[-count:],
            "values": chart.get("values", [])[-count:],
            "by_model": {n: v[-count:] for n, v in chart.get("by_model", {}).items()},
        }

    def render_chart(self, animate=True):
        chart = self.current_series()
        values = chart.get("values", [])
        labels = chart.get("labels", [])
        daily = self.scope == "all"
        self.range_box.setVisible(daily)
        self.rate.setVisible(not daily)
        self.chart_label.setText("每日 Token" if daily else "Token / min")
        self.chart.set_series(values, labels, animate, "每日 Token" if daily else "Token / min")
        self.rate.setText(display_number(values[-1]) if values else "—")
        self.mini_chart.set_series(values, labels, animate, "每日 Token" if daily else "Token / min")
        if daily:
            for name, row in self.rows.items():
                row.chart.set_series(chart.get("by_model", {}).get(name, []), animate=animate)
    ...
    def render_quota(self):
        ...
        if not quotas:
            ...
            self.quota_reset.setText("")
            self.mini_quota.setText("—")
            return
        title, row = quotas[0]
        ...
        if len(quotas) > 1:
            ...
            if hour:
                self.quota_reset.setText(f'5h 剩余 {100-float(hour["used_percent"]):g}%')
    ...
    def change_scope(self, scope):
        self.scope = scope
        for b, key in zip(self.group.buttons(), ("today", "last_5_hours", "all")):
            b.setChecked(scope == key)
            b.setFont(font(16, scope == key))
        self.render()
    ...
    def set_motion(self, value):
        ...
        for chart in [self.chart, self.mini_chart] + [r.chart for r in self.rows.values()]:
            ...
            if not value:
                ...
                chart.display_values = chart.target_values[:]
                chart.pulse.stop()
                chart.update()
        if value:
            self.update_status()
    ...
    def save_settings(self):
        if self.persist:
            ...
            self.settings.setValue("position", host.pos())
            self.settings.setValue("transparency", self.transparency)
            self.settings.setValue("preset", self.glass_preset)
            if host is not self:
                self.settings.setValue("scale", host.scale)
    ...
    def toggle_top(self, checked):
        ...
        if hasattr(self, "appearance"):
            self.appearance.top.blockSignals(True)
            self.appearance.top.setChecked(checked)
            self.appearance.top.blockSignals(False)

    def toggle_compact(self):
        self.compact_mode = not self.compact_mode
        self.body.setVisible(not self.compact_mode)
        self.cost_title.setVisible(not self.compact_mode)
        ...
        if host:
            host.sync_size()
            host.clamp_to_screen()
        self.fit_metrics()
        self.update()

    def show_menu(self):
        self.menu.popup(self.global_point(self.more, QPoint(-120, 30)))

    def show_appearance(self):
        point = self.global_point(self, QPoint(-340, 70))
        screen = QApplication.screenAt(self.host_window().frameGeometry().center()) or QApplication.primaryScreen()
        area = screen.availableGeometry()
        if point.x() < area.left():
            point = self.global_point(self, QPoint(100, 90))
        self.appearance.adjustSize()
        point.setX(min(max(point.x(), area.left()), max(area.left(), area.right() - self.appearance.width() + 1)))
        point.setY(min(max(point.y(), area.top()), max(area.top(), area.bottom() - self.appearance.height() + 1)))
        self.appearance.move(point)
        self.appearance.show()
        self.appearance.raise_()

    def select_model(self, name):
        self.selected = name
        _, models = window_data(self.data, self.scope)
        r = models.get(name, {})
        self.detail_popup.setText(
        ...
        point = self.global_point(self, QPoint(30, 410))
        area = (QApplication.screenAt(point) or QApplication.primaryScreen()).availableGeometry()
        point.setX(max(area.left(), min(point.x(), area.right() - self.detail_popup.width() + 1)))
        point.setY(max(area.top(), min(point.y(), area.bottom() - self.detail_popup.height() + 1)))
        self.detail_popup.move(point)
        self.detail_popup.show()
        QTimer.singleShot(6500, self.detail_popup.hide)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton and event.y() < 65:
    ...
    def mouseReleaseEvent(self, event):
        if self.drag is not None:
            host = self.host_window()
            area = (
                QApplication.screenAt(host.frameGeometry().center()) or QApplication.primaryScreen()
            ).availableGeometry()
            host.move(
                max(area.left(), min(host.x(), area.right() - host.width() + 1)),
                max(area.top(), min(host.y(), area.bottom() - host.height() + 1)),
            )
            self.save_settings()
        self.drag = None
    ...
    def tray_activate(self, reason):
        if reason == QSystemTrayIcon.Trigger:
            self.hide_to_tray() if self.host_window().isVisible() else self.restore()
    ...
    def open_dashboard(self):
        from codex_glass.desktop.dashboard import open_dashboard
    ...
    def launch_backend(self):
        ...
        if self.backend_process is not None and self.backend_process.poll() is None:
            return
        if self.backend_attempts >= 3 or time.monotonic() < self.backend_next_attempt:
            return
        ...
        try:
            ...
            command = (
                [sys.executable, "--backend", "--no-browser"]
            ...
            with (runtime / "backend.log").open("a", encoding="utf-8") as log:
                self.backend_process = subprocess.Popen(
                    command,
                    cwd=str(Path(sys.executable).parent if getattr(sys, "frozen", False) else resource_root()),
                    stdout=log,
                    stderr=log,
            ...
            self.dot.setToolTip("后台启动失败：" + str(exc))
    ...
    def fetch(self):
        ...
        request = QNetworkRequest(QUrl(self.url + "/api/widget"))
...
def main():
    if "--backend" in sys.argv:
        ...
        if sys.stdout is None:
            sys.stdout = (runtime / "backend.log").open("a", encoding="utf-8")
    ...
    parser.add_argument("--scope", choices=["today", "last_5_hours", "all"], default="today")
    ...
    if not args.capture:
        ...
        if socket.waitForConnected(500):
            ...
            return 0
        server = QLocalServer()
        QLocalServer.removeServer("codex-glass-frosted-v3")
        if not server.listen("codex-glass-frosted-v3"):
            return 1
    widget = GlassWidget(args.url, not args.no_start_backend, persist=not bool(args.capture))
    widget.change_scope(args.scope)
    window = widget
    if not args.capture:
