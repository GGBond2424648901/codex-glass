...
from codex_glass.core.presentation import compact, window_data

COLORS = ["#08c6e7", "#994cff", "#ff836a", "#159cff", "#8794b5", "#21cdb1"]
KNOWN_COLORS = {
    "gpt-6-astra": COLORS[0],
    "gpt-5.6-sol": COLORS[1],
    "gpt-5.6-terra": COLORS[2],
    "gpt-5.6-luna": COLORS[3],
    "unknown": COLORS[4],
    "unknown-model": COLORS[4],
}
...
def token_structure(row):
    inputs = max(0, row.get("input_tokens", 0))
    cached = min(inputs, max(0, row.get("cached_input_tokens", 0)))
...
def scope_bounds(scope, now=None):
    ...
    return (
        "" if scope == "all" else start.isoformat(sep=" ", timespec="seconds"),
...
def model_rows(data, scope, query="", selected=None, workspace=False):
    ...
    for name, stats in models.items():
        if query.casefold() not in name.casefold() or (selected is not None and name not in selected):
            continue
        rows.append(
            dict(
                stats,
...
def export_events(path, events):
    ...
    with open(path, "w", encoding="utf-8-sig", newline="") as stream:
        ...
        writer.writerow(["时间", "模型", "工作区", "输入", "缓存", "输出", "推理", "Token", "预估费用USD"])
        for row in events:
            t = row.get("tokens", {})
            writer.writerow(
                [safe(row.get(k, "")) for k in ("timestamp", "model", "cwd")]
                + [t.get(k, 0) for k in ("input", "cached_input", "output", "reasoning_output", "total")]
                + ["未计价" if row.get("pricing_source") == "unpriced" else row.get("cost_usd", {}).get("total", 0)]
            )
