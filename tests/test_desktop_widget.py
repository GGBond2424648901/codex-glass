...
from codex_monitor_core import MonitorConfig, UsageEvent, UsageDelta, aggregate_usage_events


class WidgetDataTests(unittest.TestCase):
    def test_pro_only_shows_weekly_but_plus_can_show_both(self):
        from widget_data import quota_windows
        ...
        self.assertEqual(['周额度','5 小时额度'],[x[0] for x in quota_windows({'rate_limits':{'limits':[week,hour]}})])
    ...
    def test_today_excludes_yesterday_and_keeps_every_model(self):
        events = [UsageEvent(datetime(2026,9,8,10), f'model-{i}', None,
                            UsageDelta(100,20,10,2,110),0,'unpriced') for i in range(20)]
        events.append(UsageEvent(datetime(2026,9,7,23), 'yesterday',None,
                                 UsageDelta(100,20,10,2,110),0,'unpriced'))
        result = aggregate_usage_events(events, [], MonitorConfig.load(Path('absent.json')),
    ...
    def test_delta_baseline_repeat_and_counter_reset(self):
        from widget_data import DeltaTracker
        ...
        new=sample(30,'five'); new['by_model']['new-model']={'total_tokens':80}
    ...
    def test_import_change_resets_delta_baseline(self):
        ...
        def sample(n,stamp,imports):
            return {'generated_at':stamp,'by_model':{'x':{'total_tokens':n}},
    ...
    def test_desktop_snapshot_passes_cached_chart_series(self):
        ...
        charts={'today':{'labels':['x'],'values':[1.0],'by_model':{},'unit':'Token / min','bucket_minutes':5}}
    ...
    def test_latest_local_record_preserves_both_windows_without_old_account_mix(self):
        from widget_data import desktop_snapshot, quota_windows
        ...
        with tempfile.TemporaryDirectory() as directory, ExitStack() as cleanup:
            root=Path(directory); index=SessionIndex(root/'index.sqlite3'); index.initialize()
            cleanup.callback(index.close)
            config=MonitorConfig.load(root/'absent.json')
            def record(stamp,plan,primary,secondary=None):
                return {'timestamp':stamp,'type':'token_count','payload':{'rate_limits':{
                    'limit_id':'codex','plan_type':plan,'primary':primary,'secondary':secondary}}}
            ...
            path=root/'sessions'/'one.jsonl'; write_jsonl(path,records)
            SessionIndexer(index,config).scan_once(path.parent)
            coordinator=SimpleNamespace(index=index,snapshot=lambda:{'total':{},'rate_limits':{'limits':[dict(week,scope='global',used_percent=99)]}})
            ...
            write_jsonl(path,records); SessionIndexer(index,config).scan_once(path.parent)
            data=desktop_snapshot(coordinator)
    ...
    def test_unchanged_legacy_index_backfills_quota_before_ready(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory); path=root/'sessions'/'one.jsonl'
            hour={'window_minutes':300,'used_percent':22,'resets_at':2100000000}
            week={'window_minutes':10080,'used_percent':17,'resets_at':2100000000}
            write_jsonl(path,[{'timestamp':'2026-09-08T01:00:00Z','type':'token_count','payload':{'rate_limits':{
                'limit_id':'codex','plan_type':'plus','primary':hour,'secondary':week}}}])
            index=SessionIndex(root/'index.sqlite3'); index.initialize()
            config=MonitorConfig.load(root/'absent.json'); SessionIndexer(index,config).scan_once(path.parent)
            index.connection.execute('DROP TABLE quota_cache'); index.connection.commit()
            index.close(); index=SessionIndex(root/'index.sqlite3'); index.initialize()
    ...
    def test_malformed_newer_quota_does_not_replace_complete_observation(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            week={'window_minutes':10080,'used_percent':17,'resets_at':2100000000}
            records=[
                {'timestamp':'2026-09-08T01:00:00Z','type':'token_count','payload':{'rate_limits':{
                    'limit_id':'codex','plan_type':'plus','primary':hour,'secondary':week}}},
                {'timestamp':'2026-09-08T02:00:00Z','type':'token_count','payload':{'rate_limits':{
                    'limit_id':'codex','plan_type':'plus','primary':{},'secondary':{'window_minutes':0,'used_percent':float('nan')}}}},
            ]
            write_jsonl(path,records)
            index=SessionIndex(root/'index.sqlite3'); index.initialize()
            config=MonitorConfig.load(root/'absent.json'); SessionIndexer(index,config).scan_once(path.parent)
    ...
    def test_legacy_backfill_tries_older_bounded_candidate_after_malformed_latest(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            index=SessionIndex(root/'index.sqlite3'); index.initialize(); config=MonitorConfig.load(root/'absent.json')
            SessionIndexer(index,config).scan_once(path.parent)
            index.connection.execute('DROP TABLE quota_cache'); index.connection.commit(); index.close()
            index=SessionIndex(root/'index.sqlite3'); index.initialize(); SessionIndexer(index,config).scan_once(path.parent)
            coordinator=SimpleNamespace(index=index,snapshot=lambda:{'total':{}})
            self.assertEqual([17,22],[row[1]['used_percent'] for row in quota_windows(desktop_snapshot(coordinator))])
            index.close()
