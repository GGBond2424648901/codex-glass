...
class NativeDashboardTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.w.setAttribute(Qt.WA_DontShowOnScreen);self.w.show();self.w.apply_data(telemetry())
        self.addCleanup(self.cleanup)

    def cleanup(self):
        self.w.timer.stop();self.w.hide();self.w.deleteLater();QTest.qWait(10)

    def test_all_fifteen_states_reuse_shell_and_only_five_pages(self):
        w=self.w;rail=w.nav_buttons['overview'].geometry();segment=w.scope_tabs.geometry()
        self.assertEqual(5,len(w.pages))
        for page in ('overview','models','history','quota','settings'):
            for scope in ('today','last_5_hours','all'):
                w.change_page(page);w.change_scope(scope)
    ...
    def test_scope_changes_overview_values_and_preserves_quota(self):
        w=self.w;w.change_scope('last_5_hours')
        self.assertIn('46.82',w.pages['overview'].metrics[0].text())
        before=w.pages['quota'].remaining.text();w.change_scope('all')
    ...
    def test_model_search_and_selected_structure(self):
        w=self.w;w.change_scope('all');w.change_page('models');page=w.pages['models']
        page.search.setText('sol');self.app.processEvents()
        self.assertEqual(1,page.table.rowCount())
        self.assertEqual('gpt-5.6-sol',page.visible_rows[0]['name'])
        page.search.clear();self.app.processEvents()
    ...
    def test_settings_change_material_and_hidden_window_stops_motion(self):
        w=self.w;page=w.pages['settings'];page.opacity.setValue(80)
        self.assertEqual(80,w.transparency)
        w.hide();self.assertFalse(w.flow_timer.isActive())

    def test_history_payload_updates_real_table_and_pagination(self):
        w=self.w;p=w.pages['history'];w.change_page('history')
        p.apply_history({'events':[{'timestamp':'2026-09-08 20:00:00','model':'gpt-6-astra','cwd':'project','tokens':{'total':10},'cost_usd':{'total':.2}}],'total':21,'offset':0,'limit':7})
        self.assertEqual(1,p.table.rowCount());self.assertTrue(p.next_button.isEnabled());self.assertFalse(p.prev_button.isEnabled())
        self.assertIn('21',p.page_label.text())

    def test_corners_are_transparent_and_no_design_image_is_rendered(self):
        w=self.w;w.set_motion(False);frame=w.grab().toImage()
    ...
    def test_navigation_and_headers_have_shared_material_renderers(self):
        from dashboard_controls import NavigationButton, GlassHeader
    ...
    def test_settings_reflect_state_and_import_stays_disabled_during_job(self):
        w=self.w;w.transparency=80;w.motion=False;w.chart_motion=False;w.topmost=False
        w.can_import=True;w.jobs['import']=object();w.render_pages();p=w.pages['settings']
    ...
    def test_live_poll_does_not_rebuild_hidden_pages(self):
        ...
        with patch.object(p,'render',wraps=p.render) as render:
            self.w.apply_data(telemetry());render.assert_not_called()
            self.w.change_page('models');render.assert_called_once()

    def test_index_failure_preserves_facts_but_is_not_live(self):
        data=telemetry();data['index']={'status':'error','last_error':'scan failed'}
    ...
    def test_quota_fetch_failure_never_uses_imported_summary_snapshot(self):
        from unittest.mock import patch
        data=telemetry();w=self.w;w.auto_fetch=True
        def run(key,operation):
            if key=='data':w.apply_data(operation())
        with patch.object(w,'get_json',side_effect=[data,RuntimeError('widget unavailable')]),patch.object(w,'run_job',side_effect=run):w.fetch()
    ...
    def test_reopening_page_choices_does_not_retain_old_widget_trees(self):
        ...
        for i in range(20):page.choose_models();page.popup.hide()
    ...
    def test_quota_observation_freshness_is_explicit(self):
        from dashboard_views import snapshot_status
    ...
    def test_import_retries_only_lease_contention(self):
        from native_dashboard import retry_import
        from unittest.mock import Mock
        action=Mock(side_effect=[RuntimeError('Another process holds the indexing lease'),{'ok':True}]);pause=Mock()
        self.assertEqual({'ok':True},retry_import(action,pause=pause))
        self.assertEqual(2,action.call_count);pause.assert_called_once()
        with self.assertRaises(ValueError):retry_import(Mock(side_effect=ValueError('invalid')),pause=pause)
    ...
    def test_header_tooltips_use_shared_glass_popup(self):
        ...
        router=install_glass_tooltips(self.w);button=self.w.refresh
        event=QHelpEvent(QEvent.ToolTip,QPoint(10,10),QPoint(400,300))
        self.app.sendEvent(button,event)
        self.assertTrue(router.popup.isVisible());self.assertEqual(button.toolTip(),router.popup.text())
        self.assertIsNone(router.popup.parentWidget());router.dismiss()
    ...
    def test_native_import_workflow_merges_and_repeated_import_deduplicates(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            root=Path(folder);source=root/'source.sqlite3';target=root/'target.sqlite3'
            write_jsonl(root/'sessions/one.jsonl',sample_records());cfg=MonitorConfig.load(root/'config.json')
            index=SessionIndex(source);index.initialize();SessionIndexer(index,cfg).scan_once(root/'sessions');index.close()
            index=SessionIndex(target);index.initialize();index.close()
            with patch('native_dashboard.QFileDialog.getOpenFileName',return_value=(str(source),'')),patch('native_dashboard.GlassDialog') as dialog:
                dialog.return_value.exec_.return_value=QDialog.Accepted
                for i in range(2):
                    self.w.context={'index_path':str(target)};self.w.can_import=True;self.w.import_history()
                    deadline=time.monotonic()+10
                    while 'import' in self.w.jobs and time.monotonic()<deadline:QTest.qWait(20)
                    self.assertNotIn('import',self.w.jobs)
                index=SessionIndex(target)
                try:
                    index.initialize();data=index.build_summary(cfg)
                    self.assertEqual(195,data['total']['total_tokens']);self.assertEqual(2,data['total']['calls'])
                finally:index.close()
    ...
    def test_busy_history_invalidates_old_navigation_result(self):
        w=self.w;w.auto_fetch=True;w.jobs['history']=object();before=w.history_generation
        w.fetch_history();self.assertGreater(w.history_generation,before)

    def test_material_has_visible_pastel_variation_and_moves_inside_mask(self):
        w=self.w;w.set_motion(False);first=w.grab().toImage();w.flow_phase=2.0;second=w.grab().toImage()
    ...
    def test_first_live_payload_applies_today_default_range(self):
        ...
        w=self.w;p=w.pages['overview'];p.chart.set_series([],[]);w.apply_data(dashboard_fixture()[0])
        self.assertEqual((54.,60.),p.chart.view_range)

    def test_large_axis_labels_are_not_clipped(self):
        from PyQt5.QtGui import QFontMetrics
        from glass_ui import font
        chart=self.w.pages['overview'].chart;chart.set_series([360000]*5,animate=False)
        needed=max(QFontMetrics(font(18)).horizontalAdvance(chart._compact_number(chart._visible_peak()*i/4)) for i in range(5))
    ...
    def test_native_dialog_is_not_embedded_and_import_result_uses_real_counts(self):
        from native_dashboard import GlassDialog
        from PyQt5.QtWidgets import QGraphicsScene
        from unittest.mock import patch
        scene=QGraphicsScene();parent=__import__('PyQt5.QtWidgets',fromlist=['QWidget']).QWidget();scene.addWidget(parent)
        dialog=GlassDialog('完成','测试',parent);self.assertIsNone(dialog.parentWidget());dialog.deleteLater()
        with patch('native_dashboard.GlassDialog') as kind:
            self.w.job_finished('import',{'imported_usage_events':123,'duplicate_usage_events':456},None)
            self.assertIn('123',kind.call_args.args[1]);self.assertIn('456',kind.call_args.args[1])
