...
w = GlassWidget("http://127.0.0.1:1", False, False)
w.set_motion(False)
w.timer.stop()
...
w.appearance.show()
QTest.qWait(30)
w.appearance.grab().save(str(out / "appearance.png"))
w.appearance.hide()
sheet = QImage(1320, 686, QImage.Format_ARGB32)
sheet.fill(Qt.transparent)
p = QPainter(sheet)
for i, pix in enumerate(frames):
    p.drawPixmap(i * 440, 0, pix)
p.end()
sheet.save(str(out / "three-pages.png"))
...
for i in range(48):
    ...
    w.update()
    QTest.qWait(1)
    w.grab().save(str(flow / f"{i:03d}.png"))
w.hide()
w.tray.hide()
w.detail_popup.hide()
